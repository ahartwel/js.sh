...
# Get the supplied username and load the config script
...
js_order=$(<"$path_to_js_source"/script_order.txt)
js_array=($js_order)
cnt=${#js_array[@]}
...
for ((i=0;i<cnt;i++)); do
    ...
    java -jar "$path_to_closure_compiler"/compiler.jar --js ${js_array[i]} --js_output_file "$tmp_js"
...
for ((i=0;i<num_compilations;i++)); do
    echo "<script src='$web_path_to_js/app.min.$i.js?v=$timestamp'></script>" >> "$path_to_js_compiled"/js.php 
done
echo "Created js.php"
